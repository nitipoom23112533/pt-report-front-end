...
export class TransactionComponent {
  ...
  fetchData() {
    ...
    this.apiService.getFilteredDataTransaction(formattedStartDate, formattedEndDate,this.selectedAllProfile).subscribe({
      next: (count) => {
        ...
        //   'FAMILYFOCUS','EARLYINCAREER','LOWERMASS','STUDENT','RETIREHIGHWEALTH','RETIREMEDIUMWEALTH','RETIRELOWWEALTH','NEWCUST3MTH','OTH','N/A','Total'];
        this.CustomerSegmentArray = ['PRECIOUSPLUS','PRECIOUS','PREWEALTH','AFFUIENTTOBE','RETIREPLANNER','BUILDUPFORFEATURE',
  ...
  exportToExcel() {
    ...
    const formattedStartDate = this.datePipe.transform(this.startDate, 'yyyy-MM-dd') ?? '';
    const formattedEndDate = this.datePipe.transform(this.endDate, 'yyyy-MM-dd') ?? '';
    // const fileNmae = 'Invitation' + '_' + currentDate + '.xlsx';
    const fileNmae = 'Transaction' + '_' + formattedStartDate+' to '+ formattedEndDate + '.xlsx';
